...
# Ide kéne általános előzetes leírás az adathalmazról, kb ami a honlapon is fent van, meg amit projekt indító doksiba is beleírtunk.
...
```{r mod}
ggplot(news[shares < 1000,], aes(x = shares%%10)) + geom_histogram()
ggplot(news[shares > 1000,], aes(x = shares%%10)) + geom_histogram()
```
...
channel_data <- melt(channel_data,measure.vars = c("mean_shares", "median_shares"), id.vars = "variable" )
ggplot(channel_data, aes(x = variable)) + geom_bar(aes(y = value, fill = `variable.1`), stat = "identity", position = "dodge")
```
...
weekday_data <- weekday_data[,c("monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday") :=
                                 list(monday * shares, tuesday * shares,
                                      wednesday * shares, thursday * shares, friday * shares, saturday * shares, sunday * shares)]
...
weekday_data <- melt(weekday_data,measure.vars = c("mean_shares", "median_shares"), id.vars = "variable" )
ggplot(weekday_data, aes(x = variable)) + geom_bar(aes(y = value, fill = `variable.1`), stat = "identity", position = "dodge")
```

#Interdependencies betweek the columns LDA_..

```{r lda_distributions}
lda <- news[,lda_columns, with = F]
plot(lda)
